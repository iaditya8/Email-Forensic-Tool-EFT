...
def compute_file_hashes(file_path: Union[str, Path], chunk_size: int = 65536) -> Dict[str, str]:
    """Compute MD5, SHA-1, SHA-256, and SHA-512 hashes of a file in strict read-only mode.
    ...
    Returns:
        Dictionary containing hexadecimal digests: {"md5": ..., "sha1": ..., "sha256": ..., "sha512": ...}
    """
    path = Path(file_path)
    if not path.is_file():
...
def compute_bytes_hashes(data: bytes) -> Dict[str, str]:
    """Compute MD5, SHA-1, SHA-256, and SHA-512 hashes for in-memory byte data.
